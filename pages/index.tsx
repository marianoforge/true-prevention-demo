...
const IndexPage: NextPage = () => {
  ...
  const [selectedCategory, setSelectedCategory] = useState<GameCategory>("COGNITIVE");
  const [showGameModal, setShowGameModal] = useState(false);
  const [currentGameKey, setCurrentGameKey] = useState<string>("");
  const [showDescriptionModal, setShowDescriptionModal] = useState(false);
  const [selectedGameForDescription, setSelectedGameForDescription] = useState<CogniFitGame | null>(null);
  
  // Estado de paginación por categoría
  const [currentPages, setCurrentPages] = useState<Record<GameCategory, number>>({
    COGNITIVE: 1,
    MATH: 1,
    LANG: 1,
  });
  ...
  useEffect(() => {
    const handleGameCompletion = (event: MessageEvent) => {
      if (event.data.type === 'cognifitGame' && event.data.status === 'completed') {
        ...
        {!isAuthenticated && (
          <div className="mt-6 p-6 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg border border-yellow-200 dark:border-yellow-800">
            ...
            <p className="text-yellow-700 dark:text-yellow-300 mb-4">
              Crea un usuario demo para acceder a todos los juegos de entrenamiento cognitivo.
            </p>
            <Button
              color="warning"
              size="lg"
          ...
          {Object.entries(categoryMapping).map(([key, info]) => {
            ...
            return (
              <Button
                ...
                size="sm"
                color="primary"
                variant={selectedCategory === categoryKey ? "solid" : "bordered"}
          ...
          <div className="mt-8 flex flex-col items-center gap-4">
            ...
            <div className="text-center">
              <p className="text-gray-600 dark:text-gray-400">
                Mostrando {startIndex + 1}-{Math.min(endIndex, allFilteredGames.length)} de {allFilteredGames.length} juegos
              </p>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg max-w-2xl w-full max-h-[80vh] overflow-hidden">
              <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
                <h3 className="text-xl font-bold text-gray-900 dark:text-white">
                  {selectedGameForDescription.assets.titles.es || selectedGameForDescription.assets.titles.en || selectedGameForDescription.key}
                </h3>
                <Button
                  size="sm"
                  variant="light"
              ...
              <div className="p-6 overflow-y-auto max-h-[60vh]">
                <div className="mb-4">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className={`px-3 py-1 rounded-full text-sm font-medium text-white ${
                      selectedGameForDescription.category === 'COGNITIVE' ? 'bg-blue-500' :
                      selectedGameForDescription.category === 'MATH' ? 'bg-green-500' :
                      selectedGameForDescription.category === 'LANG' ? 'bg-purple-500' : 'bg-blue-500'
                    }`}>
                      {selectedGameForDescription.category === 'COGNITIVE' ? 'Juego Cognitivo' :
                       selectedGameForDescription.category === 'MATH' ? 'Juego Matemático' :
                       selectedGameForDescription.category === 'LANG' ? 'Juego de Lenguaje' : 'Juego Cognitivo'}
                    </span>
                  </div>
                ...
                <div className="mb-6">
                  <h4 className="font-semibold text-lg mb-3 text-gray-900 dark:text-white">Descripción completa:</h4>
                ...
                {selectedGameForDescription.skills && selectedGameForDescription.skills.length > 0 && (
                  <div className="mb-6">
                    <h4 className="font-semibold text-lg mb-3 text-gray-900 dark:text-white">Habilidades cognitivas:</h4>
                    <div className="flex flex-wrap gap-2">
                      {selectedGameForDescription.skills.map((skill, index) => (
                        ...
                        >
                          {skill.replace(/_/g, " ").toLowerCase().split(" ")
